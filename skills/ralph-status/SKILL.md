...
Output is a sectioned table — Done / Running / Queued — for the latest ralph run. See `docs/usage.md` "Checking progress mid-run" for the operator-facing playbook, and the spec at `docs/specs/ralph-status-command.md` if it has been written.
...
```bash
bash "$CLAUDE_PLUGIN_ROOT/skills/ralph-status/scripts/render_status.sh"
```
