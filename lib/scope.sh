#!/usr/bin/env bash
# Scope loader: parse <repo>/.sensible-ralph.json, export SENSIBLE_RALPH_PROJECTS (newline-joined).
# Usage: source scripts/lib/scope.sh
#
# Scope lives in a per-repo `.sensible-ralph.json` at the repo root and is auto-discovered
...
# `.sensible-ralph.json` `initiative` expansion path calls). The fail-loud guard at
...
#
# Exports:
#   SENSIBLE_RALPH_PROJECTS (newline-joined) — from <repo>/.sensible-ralph.json
#   SENSIBLE_RALPH_DEFAULT_BASE_BRANCH — from .sensible-ralph.json `default_base_branch` (default "main")
#   SENSIBLE_RALPH_SCOPE_LOADED — tuple "<repo-root-abs-path>|<scope-hash>"

# Resolve the working tree root for .sensible-ralph.json discovery.
#
# Uses --show-toplevel (not --git-common-dir) so each worktree reads its OWN
# committed .sensible-ralph.json. A branch that changes scope is edited in a worktree;
...
# Load <repo>/.sensible-ralph.json, validate, export SENSIBLE_RALPH_PROJECTS (newline-joined).
#
# Validation (all hard errors — no silent fallbacks; .sensible-ralph.json missing or
...
_scope_load_projects() {
  local repo_root="$1"
  local scope_file="$repo_root/.sensible-ralph.json"

  if [[ ! -f "$scope_file" ]]; then
    echo "scope: .sensible-ralph.json not found at '$scope_file' — create it with {\"projects\": [...]} or {\"initiative\": \"...\"}" >&2
    return 1
  fi
  ...
  if [[ "$has_projects" == "true" && "$has_initiative" == "true" ]]; then
    echo "scope: .sensible-ralph.json has both 'projects' and 'initiative' — pick one" >&2
    return 1
  fi

  if [[ "$has_projects" != "true" && "$has_initiative" != "true" ]]; then
    echo "scope: .sensible-ralph.json must set either 'projects' or 'initiative'" >&2
    return 1
  fi
  ...
  if [[ "$has_projects" == "true" ]]; then
    ...
    if [[ -z "$projects_newline" ]]; then
      echo "scope: .sensible-ralph.json 'projects' list is empty" >&2
      return 1
    fi
  else
  ...
  export SENSIBLE_RALPH_PROJECTS="$projects_newline"
  ...
  # behavior for every existing .sensible-ralph.json). Empty string and non-string
...
_scope_load() {
  ...
  # for the .sensible-ralph.json `initiative` shape. Fail loudly at load time so
  ...
  # repo root or the .sensible-ralph.json content hash changes. Content-hashing
  ...
  # SENSIBLE_RALPH_PROJECTS across a scope change with no signal.
  local scope_hash
  scope_hash="$(shasum -a 1 < "$repo_root/.sensible-ralph.json" | awk '{print $1}')"
  export SENSIBLE_RALPH_SCOPE_LOADED="${repo_root}|${scope_hash}"
}
