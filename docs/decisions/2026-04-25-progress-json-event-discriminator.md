...
ENG-241 added `/ralph-status`, which needs to distinguish dispatch-start
...
`.ralph/progress.json`, leaving the outer structure (a flat JSON array,
...
- **Separate start file** (`.ralph/running.json`): two files to maintain
...
(no `event` field) is handled by `/ralph-status` via run_id filtering: the
latest run always has new-schema records, so legacy records from older runs
are never reachable.

## Consequences

- `/ralph-status` must filter by `event` before classifying. The jq idiom is
...
- Pre-ENG-241 records (no `event` field) remain readable by `jq` but are
  filtered out by `/ralph-status` before event-discrimination — they render as
