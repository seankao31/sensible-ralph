...
For an `INTEGRATION ...` base, `lib/worktree.sh::worktree_create_with_integration` (create path) or `worktree_merge_parents` (reuse path) creates/operates on the worktree and merges parents sequentially. Conflict handling is uniform across parent count and identical across both helpers:

- **Any conflict (single or multi parent):** the worktree is left with unresolved markers and the helpers return 0. Both helpers write `.sensible-ralph-pending-merges` — a marker file listing the full original parent list pinned to commit SHAs, in original order. The dispatched session reads the marker, validates it fail-closed, resolves conflicts, completes any in-progress merge, and re-invokes `worktree_merge_parents` with the marker SHAs to drain remaining parents. The ancestor-skip in both helpers' merge loops makes re-invocations idempotent. See `docs/design/worktree-contract.md` "Pending parent merges" for the full protocol.
- **Genuine merge error (unrelated histories, object unreachable, etc.):** the helper returns non-zero with no marker. The orchestrator records `setup_failed` with `failed_step = worktree_create_with_integration` (create path) or `worktree_merge_parents` (reuse path), applies `ralph-failed`, and taints descendants.

The prior single/multi asymmetry has been removed (ENG-282). The former abort-on-multi-parent behavior is documented in `docs/archive/decisions/ralph-v2-multi-parent-integration-abort.md` as historical context.

`.sensible-ralph-base-sha` is captured **post-merge** in all paths and shapes — it's `git rev-parse HEAD` of the worktree after worktree creation/merge completes. On a clean merge, spec commits and merged parent commits are ancestors of base-sha and are excluded from `/prepare-for-review`'s impl diff. On a conflict (any parent count), HEAD has not advanced, base-sha = pre-merge spec HEAD — the agent's resolution commit and any subsequent parent merges land after base-sha and appear in the review diff, which is intentional.
...
6. **Worktree setup**, branching on the state from step 5:
   - **`both_exist` (reuse path):** `worktree_merge_parents "$path" "${parents[@]}"` merges any in-review parents into the existing branch. On any conflict (single or multi parent), the helper leaves the worktree in MERGING state, writes the `.sensible-ralph-pending-merges` marker, and returns 0. The orchestrator logs a notice and proceeds with base-sha capture and dispatch; the agent drains the marker per `/sr-implement` Step 2. Genuine merge errors return non-zero → `setup_failed`. No worktree teardown on the reuse path — the existing branch+worktree predate this invocation.
...
- `docs/archive/decisions/ralph-v2-multi-parent-integration-abort.md` — historical rationale for the former multi-parent abort behavior (superseded by ENG-282, which replaced abort with leave-in-place + pending-merges marker).
