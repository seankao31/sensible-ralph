...
3. **Bounded silent-rejoin invariant — applies to ALL labeled-failure paths in this PR.** Across the dispatched-outcome paths (`failed`, `exit_clean_no_review`) AND the `setup_failed` path, **assuming no concurrent unintended label mutation between verify and revert** (Fix shape > Concurrency assumption) AND **assuming Linear read-after-write consistency for sequential same-session API calls** (Fix shape > Linear consistency assumption), no failure path produces an `Approved`-state issue without the `ralph-failed` label. The verify-after-add gate is what guarantees this — Linear's silent-no-op on a missing workspace label is detected and the helper returns failure rather than letting the issue silently rejoin. For the dispatched-outcome paths, the gate also conditions the state revert (so `Approved` is reached only when the label is verified). For the `setup_failed` path, no state revert is involved (state was never written to `In Progress`); the gate's value there is purely the silent-no-op detection logged to stderr. The unlabeled `In Progress` degraded outcomes (criterion 2c) do not violate this invariant because state is not `Approved`.
