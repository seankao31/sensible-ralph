...
- **System-side surfacing AND operator recovery doc for stuck `In Progress`.** The verify-fail and label-fail recovery paths leave the issue `In Progress` with no marker (acceptance criterion 2c). Operator board inspection is the recovery signal today. ENG-322 does NOT ship a recovery doc for this class because operator-facing recovery without a system-side surfacing path would ask the operator to manually scan `progress.json` after every run for a rare anomaly. Both deliverables — the system-side surfacing AND the operator recovery flow — land together in **ENG-327** (filed as a follow-up to ENG-322; related to ENG-254 crash detection since the same path-survey mechanism catches both classes). Filing as a single follow-up keeps scope discipline here AND avoids stranding 2c on a half-shipped recovery story.

  **Codex review note (override):** During the spec's adversarial review (`/sr-spec` step 10), codex repeatedly argued that 2c's lack of durable surfacing makes ENG-322 "operationally unsafe" to ship. Sean and I overrode that finding as bucket 3 (user judgment / contradicts prior dialogue). Rationale: (a) ENG-322 does not regress safety — the silent-no-op already strands issues today; the verify gate turns it into a stderr-visible event, a strict improvement; (b) the compound failure (Linear API blip + missed stderr + missed board inspection) is rare; (c) the same path-survey infrastructure ENG-254 needs would handle 2c, so bundling avoids divergent designs; (d) the spec went through eight codex iterations and the findings were converging on edge precision rather than fundamental issues. The override is recorded here so that future readers don't infer from the issue closing in `Approved` that 2c was "fixed" — it was knowingly deferred to ENG-327.
...
- **`skills/sr-start/SKILL.md:110`** (live operator-facing skill text — NOT just archival docs). Currently reads "Decide: retry (remove the `ralph-failed` label and re-queue), cancel the issue, or debug interactively." Update to mirror `docs/usage.md:15`'s wording: "Decide: retry (remove the `ralph-failed` label — the orchestrator typically reverts state to `Approved` automatically; on the rare path where the revert call failed, also flip state to `Approved` manually — see `docs/design/linear-lifecycle.md` for the full triage recipe), cancel the issue, or debug interactively." This skill is the live surface invoked when the operator runs `/sr-start`; leaving the old wording here would have an autonomous implementer or operator following stale guidance for the very change we're shipping.

Acceptance grep checks BOTH `docs/` and `skills/` (and `README.md`) for the old "remove the failed label and re-queue" / "remove the `ralph-failed` label … re-queue" wording. The phrase must not appear in any live operator-facing surface after the change.
