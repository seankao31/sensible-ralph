...
Moving both into `.ralph/` tidies the repo root and lets the gitignore
collapse from a list of filenames to a single directory entry. The new
directory coexists with the existing `.ralph.json` consumer scope file
...
`Agent Config` Linear project (out of scope for this repo's `.ralph.json`),
...
The `/.ralph/` entry covers both `.ralph/progress.json` and
`.ralph/ordered_queue.txt`. It does NOT match `.ralph.json` at the repo
...
> repo root, run `mkdir -p .ralph && mv progress.json ordered_queue.txt
> .ralph/ 2>/dev/null` once at your consumer repo's root.

### 3. `skills/ralph-start/scripts/orchestrator.sh`
...
# After:
mkdir -p .ralph
"$SKILL_DIR/scripts/build_queue.sh" > .ralph/ordered_queue.txt
```
...
# After:
"$SKILL_DIR/scripts/orchestrator.sh" .ralph/ordered_queue.txt
```

(c) Lines 94, 102, 105, 106 — narrative references to `progress.json`'s
location. Update to `.ralph/progress.json`. Field-name references
(`failed_step`, `residue_path`, etc.) are unchanged.

### 5. `skills/ralph-start/scripts/build_queue.sh`
...
and queue files`. Update `progress.json` → `.ralph/progress.json`. The
phrase "queue files" is generic and stays as-is.

(b) Line 15 — "Inspect `progress.json` at the repo root" →
"Inspect `.ralph/progress.json` at the repo root".

### 8. `skills/ralph-start/scripts/test/orchestrator.bats`

`grep -n 'progress\.json' skills/ralph-start/scripts/test/orchestrator.bats`
returns ~36 hits across the file. Rewrite every path expression
(`$REPO_DIR/progress.json` → `$REPO_DIR/.ralph/progress.json`) and
...
Update each location-bearing reference to the `.ralph/`-prefixed path.
...
- The `ralph` → `sensible-ralph` identity rename. Filed as a separate
  ...
  approves; specced later in its own `/ralph-spec` dialogue.
- Per-worktree artifacts (`.ralph-base-sha`, `ralph-output.log`).
...
- The three `docs/specs/*-design.md` files in `docs/specs/`
  (`ralph-loop-v2-design.md`, `ralph-implement-skill-design.md`,
...
1. `.gitignore` lists `/.ralph/` and does NOT list `/progress.json` or
...
3. From the consumer repo root with `.ralph/` existing:
   ```bash
   git check-ignore -v .ralph/progress.json .ralph/ordered_queue.txt
   ```
   Both paths resolve against the new `/.ralph/` block (output
   references `.gitignore:N:/.ralph/`).

4. A fresh `/ralph-start` invocation on a test fixture creates
   `.ralph/progress.json` (not `progress.json` at the repo root). Verify
   by running:
   ```bash
   bats skills/ralph-start/scripts/test/orchestrator.bats
   ```
   All tests pass.
...
6. `.ralph.json` (the consumer scope file at the repo root) is NOT
   ...
   different ignore source than the new `/.ralph/` block).

7. Per-worktree artifacts unchanged: `.ralph-base-sha` still written by
...
1. `bats skills/ralph-start/scripts/test/orchestrator.bats` — load-bearing
   integration check.
2. The repo-wide grep from acceptance criterion 5 — must return only
   path-independent narrative.
3. `git check-ignore -v` against a freshly-created `.ralph/` in a
...
```bash
mkdir -p .ralph
mv progress.json .ralph/ 2>/dev/null || true
rm -f ordered_queue.txt
```

`progress.json` is preserved (cross-run audit log). `ordered_queue.txt`
is regenerated on every `/ralph-start` invocation, so dropping it is
safe.
...
```
fix: relocate progress.json and ordered_queue.txt under .ralph/
...
Per-worktree artifacts (.ralph-base-sha, ralph-output.log) are
