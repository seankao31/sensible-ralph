...
**Operator decision is firm.** Codex adversarial review has
re-flagged this trade-off across multiple rounds (rounds 4–6) under
various headings ("breaks `/sr-status` for existing queues",
"upgrade path hard-breaks every repo", "no migration story"). All
of those findings are technically accurate from an adversarial-review
seat: the no-fallback contract IS a deterministic on-disk-format
break for the existing operator population (Sean's repos). The
trade-off has been reviewed and explicitly accepted on cost grounds
(one operator, one-step recovery per repo) versus benefit grounds
(simpler contract, no indefinite dead-code maintenance, no
migration code in the orchestrator). This bullet exists to record
the decision, not to invite further iteration; future code-level
follow-ups go through a separate Linear issue.
...
5. **`docs/design/orchestrator.md`** — the `progress.json` schema
   ...
   orchestrator. Behavior is unchanged under Option β: the
   orchestrator generates `run_id` in-memory at startup (the same
   `date -u +...` line, just preserved at its position) and copies
   the value into both `ordered_queue.txt`'s header and every
   `progress.json` record. The doc update is to add the header as
   a *sink* of `run_id` alongside `progress.json`, not to recharacterize
   `run_id`'s origin. **Source-of-truth contract:** the in-memory
   value generated by the orchestrator is authoritative; the queue
   header and `progress.json` are both copies. Readers (including
   `/sr-status`) consume from one of the copies; nothing derives a
   new `run_id` from a stored copy. Codex round-6 finding 3
   surfaced an internal contradiction in earlier wording of this
   bullet; this paragraph resolves it.
