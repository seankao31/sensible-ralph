...
- `orchestrator.sh` writes almost nothing to stdout; errors go to stderr; per-worktree `<RALPH_STDOUT_LOG>` captures the `claude -p` stream but is per-session, not an aggregate.
- `run_in_background` hands Claude Code a silent pipe; no notification surface.
- ENG-255 (Approved, sequenced before this ticket) relocates the orchestrator's artifacts under `.ralph/` — `.ralph/progress.json` and `.ralph/ordered_queue.txt`. This spec assumes the post-relocation paths.
...
Add a **start** record to `.ralph/progress.json` when an issue is dispatched, in addition to the existing end record. Both record types gain an `event` field discriminator.
...
A "Running" row that looks stuck across multiple `/ralph-status` invocations usually means the orchestrator died (Esc in the TUI, terminal closed, machine sleep). This skill does not detect that automatically — it shows whatever `progress.json` says. The operator confirms with `pgrep -f orchestrator.sh`.

ENG-254 (Backlog) owns proper crash detection — signal trap in the orchestrator that writes a synthetic `killed_mid_dispatch` end record, plus a PID file at `.ralph-orchestrator.pid` for liveness checks. When ENG-254 lands, `/ralph-status` will be enriched in two small patches:

- Recognize the new `killed_mid_dispatch` outcome → display as `INTERRUPTED` instead of falling through to the Running section.
- Read `.ralph-orchestrator.pid` and run `kill -0 $pid 2>/dev/null` → annotate Running rows with `(orchestrator alive)` or `(orchestrator dead)`.
...
Best-effort: matches the existing pattern. `_progress_append` failure does not abort dispatch — the orchestrator continues and writes the end record as usual when claude exits. /ralph-status would just be missing this issue from the Running count for the gap window.
...
---
name: ralph-status
description: Read-only status of the current (or most recent) ralph run. Prints a sectioned table — Done / Running / Queued — read from .ralph/progress.json and .ralph/ordered_queue.txt. Zero side effects.
allowed-tools: Bash
...
> Run `/ralph-status` from anywhere inside the repo to print a sectioned table — Done / Running / Queued — for the latest ralph run. Read-only, zero side effects. Useful both during a run (to see what's currently dispatching) and after (to glance at the most recent run's outcomes without having to `jq` `.ralph/progress.json` by hand).
>
...
Also update `skills/ralph-start/SKILL.md`'s "What to expect" pointer to mention `/ralph-status` as a faster alternative to manual `jq` inspection.

## Test coverage

### Additions to `skills/ralph-start/scripts/test/orchestrator.bats`

1. **Start record present after dispatch.** The single-issue success test additionally asserts that `.ralph/progress.json` contains a record with `event == "start"`, `issue == "ENG-10"`, `run_id == <run>`. Verify the start record's array index is lower than the end record's (start written before end).
...
### New file `skills/ralph-status/scripts/test/render_status.bats`
...
7. **Mixed Done + Running + Queued** → all three sections populated correctly, Done row uses end-record fields, Running row uses start-record fields, Queued list reads from `.ralph/ordered_queue.txt` minus Done/Running.
...
1. `orchestrator.sh::_dispatch_issue` writes a start record (`event: "start"`, `issue`, `branch`, `base`, `timestamp`, `run_id`) to `.ralph/progress.json` after `linear_set_state` succeeds and before `claude -p` is invoked.
...
4. `skills/ralph-status/SKILL.md` exists with `disable-model-invocation: true`, no arguments, `allowed-tools: Bash`.
5. `skills/ralph-status/scripts/render_status.sh` reads `.ralph/progress.json` and `.ralph/ordered_queue.txt`, partitions records from the latest `run_id` into Done / Running / Queued sections, and prints the table format specified above. Zero writes to Linear, git, the filesystem. No network calls.
6. Bats coverage updated per the test list above. All bats tests pass: `bats skills/ralph-start/scripts/test/orchestrator.bats skills/ralph-status/scripts/test/render_status.bats`.
7. `docs/usage.md` documents `/ralph-status` under a "Checking progress mid-run" subsection. `skills/ralph-start/SKILL.md`'s "What to expect" guidance mentions `/ralph-status` as a faster alternative to manual `jq` inspection.
...
- **Linear API state lookup of in-flight issue** — decided against during /ralph-spec dialogue (network cost, marginal info: the next end record lands within a minute anyway).
...
```
feat: add /ralph-status command backed by live progress.json start/end events

Add a start record to .ralph/progress.json when each issue is dispatched,
in addition to the existing end record. Both record types gain an `event`
field discriminator (`start` / `end`).

New /ralph-status skill prints a sectioned table — Done / Running /
Queued — for the latest ralph run, read from .ralph/progress.json and
...
Branch: `eng-241-add-ralph-status-command-backed-by-live-progressjson` (Linear default).
